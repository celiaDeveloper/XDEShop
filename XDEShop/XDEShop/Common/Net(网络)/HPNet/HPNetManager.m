...
@interface HPNetManager ()

@property(nonatomic, strong) AFHTTPSessionManager *sessionManager;

@end

@implementation HPNetManager

+ (instancetype)sharedHPNetManager
{
    /*! 为单例对象创建的静态实例，置为nil，因为对象的唯一性，必须是static类型 */
    static id sharedHPNetManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedHPNetManager = [[super allocWithZone:NULL] init];
    });
    return sharedHPNetManager;
}

+ (void)initialize
{
    [self setupHPNetManager];
}

+ (void)setupHPNetManager
{
    HPNetManagerShare.sessionManager = [AFHTTPSessionManager manager];
    
    HPNetManagerShare.requestSerializer = HPHttpRequestSerializerJSON;
    HPNetManagerShare.responseSerializer = HPHttpResponseSerializerJSON;
    ...
    //    HPNetManagerShare.sessionManager.responseSerializer = response;
    ...
    //    HPNetManagerShare.sessionManager.requestSerializer = request;
    ...
    /*! 设置响应数据的基本类型 */
    HPNetManagerShare.sessionManager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/json", @"text/javascript", @"text/html", @"text/css", @"text/xml", @"text/plain", @"application/javascript", @"application/x-www-form-urlencoded", @"image/*", nil];
...
+ (void)setupSecurityPolicy
{
    ...
    if (cerSet.count == 0)
    {
        ...
        HPNetManagerShare.sessionManager.securityPolicy = securityPolicy;
    }
    else
    {
        /*! 自定义的CA证书配置如下： */
        ...
        //            HPNetManagerShare.sessionManager.securityPolicy = policy;
    }
}
...
{
    return [self requestWithType:HPHttpRequestTypeGet isNeedCache:isNeedCache urlString:urlString parameters:parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}
...
+ (HPURLSessionTask *)POSTWithUrlString:(NSString *)urlString
                                     ...
                                     progressBlock:(HPDownloadProgressBlock)progressBlock
{
    return [self requestWithType:HPHttpRequestTypePost isNeedCache:isNeedCache urlString:urlString parameters:parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}
...
+ (HPURLSessionTask *)PUTWithUrlString:(NSString *)urlString
                                    ...
                                    progressBlock:(HPDownloadProgressBlock)progressBlock
{
    return [self requestWithType:HPHttpRequestTypePut isNeedCache:NO urlString:urlString parameters:parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}
...
+ (HPURLSessionTask *)DELETEWithUrlString:(NSString *)urlString
                                       ...
                                       progressBlock:(HPDownloadProgressBlock)progressBlock
{
    return [self requestWithType:HPHttpRequestTypeDelete isNeedCache:NO urlString:urlString parameters:parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}
 ...
 *  @param progressBlock 进度
 */
+ (HPURLSessionTask *)requestWithType:(HPHttpRequestType)type
                             isNeedCache:(BOOL)isNeedCache
                           ...
                           progressBlock:(HPDownloadProgressBlock)progressBlock
{
    ...
    AFHTTPSessionManager *scc = HPNetManagerShare.sessionManager;
    AFHTTPResponseSerializer *scc2 = scc.responseSerializer;
    AFHTTPRequestSerializer *scc3 = scc.requestSerializer;
    NSTimeInterval timeoutInterval = HPNetManagerShare.timeoutInterval;
    
    NSString *isCache = isNeedCache ? @"开启":@"关闭";
    CGFloat allCacheSize = [HPNetManagerCache getAllHttpCacheSize];
    
    NSLog(@"\n******************** 请求参数 ***************************");
    NSLog(@"\n请求头: %@\n超时时间设置：%.1f 秒【默认：30秒】\nAFHTTPResponseSerializer：%@【默认：AFJSONResponseSerializer】\nAFHTTPRequestSerializer：%@【默认：AFJSONRequestSerializer】\n请求方式: %@\n请求URL: %@\n请求param: %@\n是否启用缓存：%@【默认：开启】\n目前总缓存大小：%.6fM\n", HPNetManagerShare.sessionManager.requestSerializer.HTTPRequestHeaders, timeoutInterval, scc2, scc3, requestType, URLString, parameters, isCache, allCacheSize);
    NSLog(@"\n********************************************************");
    
    HPURLSessionTask *sessionTask = nil;
    
    // 读取缓存
    id responseCacheData = [HPNetManagerCache httpCacheWithUrlString:urlString parameters:parameters];
    ...
    {
        sessionTask = [HPNetManagerShare.sessionManager GET:URLString parameters:parameters  progress:^(NSProgress * _Nonnull downloadProgress) {
            /*! 回到主线程刷新UI */
            ...
            // 对数据进行异步缓存
            NSLog(@"开始缓存数据了");
            [HPNetManagerCache setHttpCache:responseObject urlString:urlString parameters:parameters];
            [[weakSelf tasks] removeObject:sessionTask];
    ...
    else if (type == HPHttpRequestTypePost)
    {
        sessionTask = [HPNetManagerShare.sessionManager POST:URLString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
    ...
    else if (type == HPHttpRequestTypePut)
    {
        sessionTask = [HPNetManagerShare.sessionManager PUT:URLString parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
    ...
    else if (type == HPHttpRequestTypeDelete)
    {
        sessionTask = [HPNetManagerShare.sessionManager DELETE:URLString parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            if (successBlock)
            {
                successBlock(responseObject);
            }
                                    ...
                                    progressBlock:(HPUploadProgressBlock)progressBlock
{
    ...
    NSLog(@"******************** 请求参数 ***************************");
    NSLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",HPNetManagerShare.sessionManager.requestSerializer.HTTPRequestHeaders, @"POST",URLString, parameters);
    NSLog(@"********************************************************");
    
    HPURLSessionTask *sessionTask = nil;
    sessionTask = [HPNetManagerShare.sessionManager POST:URLString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
                      ...
                      progressBlock:(HPUploadProgressBlock)progressBlock
{
    ...
    [avAssetExport exportAsynchronouslyWithCompletionHandler:^{
        switch ([avAssetExport status]) {
            ...
            {
                [HPNetManagerShare.sessionManager POST:urlString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
                                     ...
                                     progressBlock:(HPDownloadProgressBlock)progressBlock
{
    ...
    NSLog(@"******************** 请求参数 ***************************");
    NSLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",HPNetManagerShare.sessionManager.requestSerializer.HTTPRequestHeaders, @"download",urlString, parameters);
    NSLog(@"******************************************************");
    
    
    HPURLSessionTask *sessionTask = nil;
    
    sessionTask = [HPNetManagerShare.sessionManager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
                                   ...
                                   progressBlock:(HPUploadProgressBlock)progressBlock
{
    ...
    NSLog(@"******************** 请求参数 ***************************");
    NSLog(@"请求头: %@\n请求方式: %@\n请求URL: %@\n请求param: %@\n\n",HPNetManagerShare.sessionManager.requestSerializer.HTTPRequestHeaders, @"uploadFile", urlString, parameters);
    NSLog(@"******************************************************");
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status)
        {
            case AFNetworkReachabilityStatusUnknown:
                NSLog(@"未知网络");
                networkStatus ? networkStatus(HPNetworkStatusUnknown) : nil;
                break;
            case AFNetworkReachabilityStatusNotReachable:
                NSLog(@"没有网络");
                networkStatus ? networkStatus(HPNetworkStatusNotReachable) : nil;
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN:
                NSLog(@"手机自带网络");
                networkStatus ? networkStatus(HPNetworkStatusReachableViaWWAN) : nil;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi:
                NSLog(@"wifi 网络");
                networkStatus ? networkStatus(HPNetworkStatusReachableViaWiFi) : nil;
                break;
        }
    }];
...
- (void)setTimeoutInterval:(NSTimeInterval)timeoutInterval
{
    ...
    HPNetManagerShare.sessionManager.requestSerializer.timeoutInterval = timeoutInterval;
}

- (void)setRequestSerializer:(HPHttpRequestSerializer)requestSerializer
{
    _requestSerializer = requestSerializer;
    switch (requestSerializer) {
        case HPHttpRequestSerializerJSON:
        {
            HPNetManagerShare.sessionManager.requestSerializer = [AFJSONRequestSerializer serializer] ;
        }
            break;
        case HPHttpRequestSerializerHTTP:
        {
            HPNetManagerShare.sessionManager.requestSerializer = [AFHTTPRequestSerializer serializer] ;
        }
            break;
...
- (void)setResponseSerializer:(HPHttpResponseSerializer)responseSerializer
{
    _responseSerializer = responseSerializer;
    switch (responseSerializer) {
        case HPHttpResponseSerializerJSON:
        {
            HPNetManagerShare.sessionManager.responseSerializer = [AFJSONResponseSerializer serializer] ;
        }
            break;
        case HPHttpResponseSerializerHTTP:
        {
            HPNetManagerShare.sessionManager.responseSerializer = [AFHTTPResponseSerializer serializer] ;
        }
            break;
...
- (void)setHttpHeaderFieldDictionary:(NSDictionary *)httpHeaderFieldDictionary
{
    ...
    for (NSInteger i = 0; i < keyArray.count; i ++)
    {
        ...
        [HPNetManager setValue:valueString forHTTPHeaderKey:keyString];
    }
}
...
{
    [HPNetManagerShare.sessionManager.requestSerializer setValue:value forHTTPHeaderField:HTTPHeaderKey];
}
...
{
    [HPNetManagerShare.sessionManager.requestSerializer clearAuthorizationHeader];
}
                             ...
                             index:(NSUInteger)index
{
    ...
    {   // 图片数据不为空才传递 fileName
        ...
        NSLog(@"上传图片 %lu-%@ 成功", (unsigned long)index,imageFileName);
    }
}
...
{
    [HPNetManagerCache clearAllHttpCache];
}
...
#ifdef DEBUG
@implementation NSArray (HPNetManager)
...
@implementation NSDictionary (HPNetManager)
